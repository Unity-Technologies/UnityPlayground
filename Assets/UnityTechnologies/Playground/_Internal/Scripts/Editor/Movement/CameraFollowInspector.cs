...
namespace Playground.Editor.Movement
{
    ...
    public class CameraFollowInspector : InspectorBase
    {
        private readonly string explanation =
            "This script makes the Camera follow a specific object (usually the Player).";

        private readonly string requiresCamera =
            "This script requires a Camera component to work. Add it to the Camera GameObject.";

        private readonly string undoLimitBoundsMessage = "Change bounds";
        private readonly string warning = "WARNING: No object is selected, so the Camera will not move.";
        ...
        public override void OnInspectorGUI()
        {
            ...
            else
            {
                ...
                if (!CheckIfAssigned("target", false)) EditorGUILayout.HelpBox(warning, MessageType.Warning);

                GUILayout.Space(5);
                GUILayout.Label("Limits", EditorStyles.boldLabel);
                bool allowLimitBoundsTemp =
                    EditorGUILayout.Toggle("Use Bounds", serializedObject.FindProperty("limitBounds").boolValue);
