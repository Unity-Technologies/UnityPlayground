...
namespace Playground.Editor.Movement
{
    ...
    {
        private readonly string arrayDataString = "waypoints.Array.data[{0}]";
        private readonly string arraySizeString = "waypoints.Array.size";
        private readonly string emptyArrayWarning = "The list of waypoints is empty, so the GameObject will not move.";
        ...
        {
            list = new ReorderableList(serializedObject, serializedObject.FindProperty("waypoints"), true, true, true,
                true);
            ...
            list.drawElementCallback =
                (rect, index, isActive, isFocused) =>
                {
                    ...
                    rect.y += 2;
                    Rect r = new(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight);
            ...
            list.onAddCallback = l =>
            {
                ...
                element.vector2Value = l.serializedProperty.GetArrayElementAtIndex(previousIndex).vector2Value +
                                       Vector2.one; //create new point, slightly offset
            };


            //draws the header of the ReorderableList
            list.drawHeaderCallback = rect => { EditorGUI.LabelField(rect, "Stops"); };
        }
        ...
        private void OnSceneGUI()
        {
            ...
            for (int i = 0; i < patrolScript.waypoints.Length; i++)
                if (!Application.isPlaying)
                {
                    ...
                    Handles.ArrowHandleCap(0, gizmoPos - orientation * 1.2f,
                        Quaternion.LookRotation(orientation, -Vector3.forward), 1f, EventType.Repaint);
            ...
            Handles.ArrowHandleCap(0, patrolScript.transform.position - orientation * 2f,
                Quaternion.LookRotation(orientation, -Vector3.forward), 1f, EventType.Repaint);
        }
