...
namespace Playground.UserInterface
{
    ...
    public class UIScript : MonoBehaviour
    {
        ...
        [Header("Configuration")] public Players numberOfPlayers = Players.OnePlayer;
        ...
        private readonly int[] playersHealth = new int[2];

        private readonly Dictionary<int, ResourceStruct>
            resourcesDict = new(); //holds a reference to all the resources collected, and to their UI
        ...
        public bool CheckIfHasResources(int resourceType, int amountNeeded = 1)
        {
            if (resourcesDict.ContainsKey(resourceType))
            {
                if (resourcesDict[resourceType].amount >= amountNeeded) return true;
