...
namespace Playground.Utilities
{
    public static class Utils
    {
        ...
        public static void DrawMoveArrowGizmo(Vector3 position, Vector2 direction, float extraAngle = 0f,
            float scale = 0f)
        {
            DrawGizmo(moveArrowMesh, position, direction, extraAngle, scale);
        }

        public static void DrawShootArrowGizmo(Vector3 position, Vector2 direction, float extraAngle = 0f,
            float scale = 0f)
        {
        ...
        public static void DrawRotateArrowGizmo(Vector3 position, float strength)
        {
            ...
            Gizmos.DrawMesh(rotateArrowMesh, position, Quaternion.identity,
                new Vector3(Mathf.Sign(strength), 1f, Mathf.Sign(strength)));
        }
        ...
        public static void DrawGizmo(Mesh meshToDraw, Vector3 position, Vector2 direction, float extraAngle,
            float scale)
        {
            Gizmos.color = Color.green;
            ...
            Gizmos.DrawMesh(meshToDraw, position, Quaternion.AngleAxis(arrowAngle + extraAngle, Vector3.forward),
                Vector3.one * scale);
        }
        ...
        public static void Collider2DDialogWindow(GameObject gameObjectRef, bool makeItTrigger = false)
        {
            ...
            if (gameObjectRef.GetComponent<Collider2D>() != null) return;

            // If not, popup a window offering a choice		
            int option = EditorUtility.DisplayDialogComplex("Collider2D needed",
