...
namespace Playground.Attributes
{
    ...
    public class DestroyForPointsAttribute : MonoBehaviour
    {
        ...
        private void Reset()
        {
            Utils.Collider2DDialogWindow(gameObject);
        }
            ...
            if (collisionData.gameObject.CompareTag("Bullet"))
            {
                if (userInterface != null)
                {
                    ...
                    if (b != null)
                        ...
                        Debug.Log(
                            "Use a BulletAttribute on one of the objects involved in the collision if you want one of the players to receive points for destroying the target.");
                }
                else
                {
